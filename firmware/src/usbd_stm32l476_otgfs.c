 ...
 */
//
// Modified 27 Aug 2018 by Mark Haun for IAR compiler, and to fix C++ type errors
//
...
USB_OTG_GlobalTypeDef * const OTG  = (USB_OTG_GlobalTypeDef *)((void*)(USB_OTG_FS_PERIPH_BASE + USB_OTG_GLOBAL_BASE)); // mh: fix for C++
USB_OTG_DeviceTypeDef * const OTGD = (USB_OTG_DeviceTypeDef *)((void*)(USB_OTG_FS_PERIPH_BASE + USB_OTG_DEVICE_BASE)); // mh: fix for C++
volatile uint32_t * const OTGPCTL  = (volatile uint32_t *)((void*)(USB_OTG_FS_PERIPH_BASE + USB_OTG_PCGCCTL_BASE));    // mh: fix for C++
...
inline static USB_OTG_INEndpointTypeDef* EPIN(uint8_t ep) {
    return (USB_OTG_INEndpointTypeDef*)((void*)(USB_OTG_FS_PERIPH_BASE + USB_OTG_IN_ENDPOINT_BASE + (ep << 5)));   // mh: fix for C++
}

inline static USB_OTG_OUTEndpointTypeDef* EPOUT(uint8_t ep) {
    return (USB_OTG_OUTEndpointTypeDef*)((void*)(USB_OTG_FS_PERIPH_BASE + USB_OTG_OUT_ENDPOINT_BASE + (ep << 5)));  // mh: fix for C++
}
...
int32_t ep_read(uint8_t ep, void* buf, uint16_t blen) {
    ...
    for (unsigned i = 0; i < len; i +=4) {
        uint32_t _t = *fifo;
        if (blen >= 4) {
            *(uint32_t __packed *)buf = _t;
            blen -= 4;
//            buf += 4;
            buf = (void *) ((char *) buf + 4);  // mh: C standard does not permit arithmetic on void *
        } else {
            while (blen){
//                *((uint8_t*)buf++) = 0xFF & _t;
                *((uint8_t*)buf) = 0xFF & _t;       // mh: C standard does not permit arithmetic on void *
                buf = (void *) ((char *) buf + 1);  // mh: C standard does not permit arithmetic on void *
...
//        return callback(dev, evt, ep);
        callback(dev, evt, ep);
        return;   // mh: void func cannot return a value
    }
}
...
uint16_t get_serialno_desc(void *buffer) {
    struct  usb_string_descriptor *dsc = (usb_string_descriptor *) buffer;  // mh: fix for C++
    char16_t __packed *str = dsc->wString;    // mh: resolve type conflict by adding "__packed"; also change uin16_t -> char16_t
