 ...
 */
//
// Modified 27 Aug 2018 by Mark Haun for IAR compiler, and to fix type errors for C++
//
...
static void usbd_process_callback (usbd_device *dev) {
    if (dev->complete_callback) {
        dev->complete_callback(dev, (usbd_ctlreq *) dev->status.data_buf);  // mh: fix for C++
...
//        dev->status.data_ptr += _t;
        dev->status.data_ptr = (void *) ((char *) dev->status.data_ptr + _t);  // mh: C standard does not permit arithmetic on void *
...
static void usbd_process_eprx(usbd_device *dev, uint8_t ep) {
    uint16_t _t;
    usbd_ctlreq *const req = (usbd_ctlreq *) dev->status.data_buf;  // mh: fix for C++
    switch (dev->status.control_state) {
    case usbd_ctl_idle:
